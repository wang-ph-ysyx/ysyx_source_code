...
void cpu_exec(unsigned long n) {
	...
	for (; n > 0; --n) {
		...
		if ((top->rootp->ysyxSoCFull__DOT__asic__DOT__cpu__DOT__cpu__DOT__lsu_awvalid || top->rootp->ysyxSoCFull__DOT__asic__DOT__cpu__DOT__cpu__DOT__lsu_arvalid) && (in_uart || in_clint)) {
			difftest_skip_ref();
		}
		if (difftest)
			difftest_step();
