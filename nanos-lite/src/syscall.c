...
#define CHANGE_PROC(a1, a2, a3) \
{ current->max_brk = 0; \
	context_uload(current, a1, a2, a3); \
...
void do_syscall(Context *c) {
  ...
  switch (a[0]) {
		case SYS_exit:  CHANGE_PROC("/bin/nterm", (char **){NULL}, (char **){NULL}) break;
		...
		case SYS_execve:if (fs_open((char *)a[1], 0, 0) < 0) c->GPRx = -2;
										else { CHANGE_PROC((char *)a[1], (char **)a[2], (char **)a[3]) }
										break;
